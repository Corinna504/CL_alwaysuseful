...
sz = get(0, 'Screensize');
fig_h = figure('Position', [sz(3)*0.2 sz(4)*0.25 sz(3)*0.6 sz(4)*0.6], 'HandleVisibility', 'callback');
...
popX_h = uicontrol(fig_h, ....
    'Style',  'popupmenu',...
    'String', [{'x'} axisopts],...
    'Position', [pos(3)*0.2 pos(4)*0.4 pos(3)*0.15 pos(4)*0.1],...
    'Callback', '');
...
pop_Xspec = uicontrol(fig_h,...
    'Style','popupmenu',...
    'String', stimulicond,...
    'Position',[pos(3)*0.2 pos(4)*0.4 pos(3)*0.08 pos(4)*0.05], ...
    'Callback', @UpdateInclusion);
    ...
    function createPlot(~, eventdata, ~)
        ...
        if (get(popY_h, 'Value') == 1) || (get(popX_h, 'Value') == 1)
            ...
            elseif (strcmp(spec.stimx, 'all stimuli cond') && strcmp(spec.stimy, 'all stimuli cond') ...
                ...
                dat.xlab = [dat.xlab ' '  spec.stimx ' ' spec.eyex];
                dat.ylab = [dat.ylab ' ' spec.stimy ' ' spec.eyey];
                
                dat = createUnitPlot(expInfo(incl_i), dat, spec, ...
...
%-------------------------------------------------------------------------
    function UpdateInclusion(check_incl, eventdata)
        ...
        UpdateInclusionHelper(1,  '~[expInfo.isadapt]');
        
        UpdateInclusionHelper(...
            strcmp(stimulicond(get(pop_Yspec, 'Value')), 'RC') & ...
            strcmp(stimulicond(get(pop_Xspec, 'Value')), 'RC'), '[expInfo.isRC]');
        
%         UpdateInclusionHelper(1, '[expInfo.lat] > 0 & [expInfo.lat_drug] > 0 ');
        ...
        UpdateInclusionHelper(1, ...
            ['[expInfo.rsqr_drug] ' get(editr2_h, 'String')]);
        UpdateInclusionHelper(1,...
