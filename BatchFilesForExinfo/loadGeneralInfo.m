...
fdir.PSTH = [pre 'Psth'];
fdir.Variability = [pre 'Variability'];
...
for i = 1:length(F_drug)
    ...
    volt = getVolt(ex2);
    [gridX, gridY, expStrt] = getGridPosition( ex0 );
    ...
    for ocul = intersect( unique([ex0.Trials.me]), unique([ex2.Trials.me]) )
        ...
        exinfo(kk).rsc_2nd = nan;         exinfo(kk).prsc_2nd = nan;
        exinfo(kk).rsc_2nd_drug = nan;    exinfo(kk).prsc_2nd_drug = nan;
        exinfo(kk).rsig_2nd = nan;        exinfo(kk).prsig_2nd = nan;
        exinfo(kk).rsig_2nd_drug = nan;   exinfo(kk).prsig_2nd_drug = nan;
        ...
        exinfo(kk).fig_varxtime = fullfile(fdir.Variability, '\', [figname '_varxtime.fig']);
        exinfo(kk).fig_noisecorr = fullfile(fdir.Variability, '\', [figname '_noisecorr.fig']);
